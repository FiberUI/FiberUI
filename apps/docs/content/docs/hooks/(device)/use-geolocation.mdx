...
description: A React hook for accessing the Geolocation API. Retrieves current coordinates, handles permission states, and tracks position changes in real-time.
keywords:
    [
        "useGeolocation",
        "react hook",
...
```npm
npx shadcn@latest add https://r.fiberui.com/r/hooks/use-geolocation.json
```
