...
description: A React hook that persists state to localStorage with automatic cross-tab synchronization. Handles serialization, hydration, and server-side rendering compatibility out of the box.
keywords:
    [
...
```npm
npx shadcn@latest add https://r.fiberui.com/r/hooks/use-local-storage-state.json
```
